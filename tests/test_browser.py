...
async def test_panel_import_validation_and_mobile():
    async with async_playwright() as pw:
        ...
        # Start remains clickable so an incomplete form explains the problem.
        await page.get_by_role("button",name="Start workers").click()
        assert "Daftar akun" in await page.locator("#batch-error").inner_text()
        ...
        for field,value in fields.items():
            if field == "country":
                await page.locator("#country").select_option(label="Indonesia")
            else:
        ...
        assert submitted[0]["identity"]["country"] == "ID"
